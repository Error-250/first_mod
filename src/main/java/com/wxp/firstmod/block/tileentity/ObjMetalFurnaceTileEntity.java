...
import com.wxp.firstmod.block.ObjMetalFurnaceBlock;
...
/** @author wxp */
public class ObjMetalFurnaceTileEntity extends TileEntity implements ITickable {
  ...
  public void update() {
    if (!this.world.isRemote) {
      ...
      if (!itemStack.isEmpty() && downInventory.insertItem(0, itemStack, true).isEmpty()) {
        this.world.setBlockState(pos, state.withProperty(ObjMetalFurnaceBlock.BURNING, Boolean.TRUE));

        int burnTotalTime;
        switch (state.getValue(ObjMetalFurnaceBlock.MATERIAL)) {
          case IRON:
            burnTotalTime = 150;
            break;
          case GOLDEN:
        ...
        this.world.setBlockState(pos, state.withProperty(ObjMetalFurnaceBlock.BURNING, Boolean.FALSE));
      }
    } else {
      IBlockState blockState = this.world.getBlockState(this.pos);
      boolean burning = blockState.getProperties().containsKey(ObjMetalFurnaceBlock.BURNING)
              && blockState.getValue(ObjMetalFurnaceBlock.BURNING);
  ...
  public Double getRotationDegree() {
    return rotationDegree * Math.PI / 180;
  }
}
